...
def main():
     ...
     if args.image_type=="MTF":
        img = mtf.SavevarMTF_XYZ(w, sj, 0, "x", normalized = 1, path=f"./", TIME_STEPS=129) 
     ...
     if  args.image_type=="GAF": 
        img = gaf.SavevarGAF_XYZ(w, sj, 0, "x", normalized = 1, path=f"./", TIME_STEPS=129) 
        imagen = cv2.imread("./1600x0gasf.png")  
        imagen = cv2.cvtColor(imagen, cv2.COLOR_BGR2RGB)
        print("Image shape",imagen.shape)
        rp=gaf.Reconstruct_GAF(imagen,dictionary,a)
     plots.plot_time_series(w,rp,0)#dimensión deber ser un valor entre 0-2
